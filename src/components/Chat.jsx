...
const Chat = () => {
  ...
  async function getRooms(filter) {
    try {
      if (filter) {
        return setFilteredRooms(rooms.filter((user) => `${user.first_name.toLowerCase()} ${user.last_name.toLowerCase()}`.includes(filter.toLowerCase())));
      }
  ...
  async function startPooling() {
    try {
      ...
      if (err.message === "TOKEN_EXPIRED") window.location.replace(`/${state.role}/login/`);
      else
        setTimeout(async () => {
          startPooling();
        }, 500);
            ...
            {showContacts && (
              <>
                ...
                <div className="lg:flex lg:w-1/3 w-full flex-2 flex-col pr-6">
                  ...
                  <div className="flex-1 h-full overflow-y-auto overflow-x-hidden px-2 max-h-[70vh] container-chat">
                    {filteredRooms &&
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex-3">
                      <h2 className="text-xl py-1 mb-8 border-b-2 border-gray-200 flex">
                        ...
                        Chatting with <b className="ml-2">{`${otherUserId.currentRoom.first_name} ${otherUserId.currentRoom.last_name}`}</b>
                      </h2>
                    </div>
                            ...
                            <div className={`flex-1 px-2 ${message?.user_id === state.user && "text-right"}`}>
                              <div className="inline-block">
                                {message.is_image ? (
                                  ...
                                  <p className={`${
                                    message?.user_id === state.user ? "bg-gray-300 text-gray-700 " : "bg-blue-600 text-white"
                                  } rounded-xl whitespace-pre-line p-2 px-6 `}>
                                    {message?.message}
                                  </p>
                                )}
                              </div>
