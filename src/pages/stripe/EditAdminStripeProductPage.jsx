...
const EditAdminStripeProductPage = () => {
  const schema = yup
    .object({
  ...
  const onSubmit = async (data) => {
    try {
      console.log(data);
      const result = await sdk.updateStripeProduct(params?.id, { name: data.name, description: data.description, status: data.status });
      if (!result.error) {
  ...
  React.useEffect(() => {
    ...
    (async function () {
      try {
        const result = await sdk.getStripeProduct(params?.id);

        if (!result.error) {
          const product = result.model.object;
...
export default EditAdminStripeProductPage;
