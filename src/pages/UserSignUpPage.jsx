...
const SignUpPage = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const result = await sdk.register(data.email, data.password, "user");
      if (!result.error) {
        dispatch({
          type: "LOGIN",
          payload: result,
  ...
  return (
    <div className="h-screen m-auto max-h-screen min-h-screen">
      <div className="min-h-full flex justify-center w-full max-h-full h-full">
        <section className="md:w-2/2 w-full flex flex-col items-center justify-center bg-white">
          <form
            onSubmit={handleSubmit(onSubmit)}
            className="flex flex-col max-w-md w-full px-6 md:mt-[6.375rem] mt-[9.375rem]"
          >
            <h1 className="md:font-bold font-semibold md:text-4xl text-3xl text-center mb-8">
              Gacha.io
            </h1>
            <h1 className="md:font-bold font-semibold md:text-2xl text-2xl text-center mb-8">
              Sign Up
            </h1>
          ...
          <div className="oauth flex flex-col gap-4 max-w-md w-full px-6 text-[#344054] grow">
            <div>
              <h3 className="mt-5 text-center text-sm text-gray-800 normal-case">
